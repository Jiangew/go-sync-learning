...
// Effective Go 总结了这么一句话：Do not communicate by sharing memory; instead, share memory by communicating.
// 在Go内存模型指出了channel作为并发控制的一个特性：A send on a channel happens before the corresponding receive from that channel completes. (Golang Spec)
package channel
